...
from numpy import zeros, identity, reshape, array, sqrt, imag, pi, where, savetxt, any, hstack, loadtxt, arange
...
def find_Lyapunov_orbit(V0, mu, T0, temporal_scheme, **kwargs):
    ...
    T0: Estimación inicial semiperiodo.
    temporal_scheme: Esquema temporal para la integración:
        - Si es str (e.g. "DOP853"), usa solve_ivp.
        - Si es callable (e.g. RK4, GBS), usa tu Cauchy_problem.
    Outputs:
    converged: booleano True si ha habido convergencia, False en caso contrario
    V0: Condición inicial de la órbita de Lyapunov
    T: Periodo de la órbita de Lyapunov
    ...
    while n_iter < N_max:
        # ─── RAMA 1: solve_ivp (tu código original, sin cambios) ──────────────────
        ...
        # ─── RAMA 2: TU ESQUEMA NUMÉRICO (nuevo bloque, equivalente) ─────────────
            ...
            for i in range(1, len(t_eval)):
                if U[i, 1] * U[i-1, 1] < 0:
                    ...
                    T_half = t_eval[i-1] + alpha * (t_eval[i] - t_eval[i-1])
                    break

            if V_cross is None:
                # Si no hay cruce, usar último punto (fallback; no ideal)
                V_cross = U[-1]
                T_half = t_eval[-1]

            if plot_intermediate_steps:
                plt.plot(U[:, 0], U[:, 1], marker="o", label="Lyapunov Orbit")
                plt.scatter(V_cross[0], V_cross[1], label="Vcross", color="green", s=100)
                plt.scatter(V0[0], V0[1], color="yellow", label="V0", s=100)
                plt.axhline(0, color='black', linewidth=1) 
                plt.axis("equal")
            ...
            raise ValueError("temporal_scheme debe ser str (para solve_ivp) o callable (tu función).")

        # ─── EL RESTO DEL CÓDIGO ES IDÉNTICO PARA AMBAS RAMAS ────────────────────
        # Paso de corrección de vy y z (usando V_cross, que ya está definido en ambas ramas)
        d1 = sqrt((V_cross[0] + mu)**2 + V_cross[1]**2 + V_cross[2]**2)
        ...
        Jacobian = zeros((2, 2))
        ddotx = (
            V_cross[0] + 2 * V_cross[4]
            - (1 - mu) * (V_cross[0] + mu) / d1**3
            - mu * (V_cross[0] - 1 + mu) / d2**3
        )
        ddotz = -((1 - mu) / d1**3 + mu / d2**3) * V_cross[2]
        ...
        V0[4] += delta_z_doty[1]  # vy_new
        V0[2] += delta_z_doty[0]  # z_new
