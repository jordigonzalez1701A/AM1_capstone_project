...
def CRTBP_variacional(V, mu, t):
    ...
    G[0:3] = V[3:6]
    G[3] = V[0] + 2*V[4]-(1-mu)*(V[0]-mu)/d1**3-mu*(V[0]-mu+1)/d2**3
    ...
    Uxy = (3*(1-mu)*(V[0]-mu)/d1**5 + 3*mu*(V[0]-mu+1)/d2**5)*V[1]
    Uxz = (3*(1-mu)*(V[0]-mu)/d1**5 + 3*mu*(V[0]-mu+1)/d2**5)*V[2]
    Uyy = -((1-mu)/d1**3 + mu/d2**3) + 3*(V[1]**2)*((1-mu)/d1**5+mu/d2**5)
    Uyz = 3*V[1]*V[2]*((1-mu)/d1**5 + mu/d2**5)
    Uzz = -((1-mu)/d1**3 + mu/d2**3) + 3*(V[2]**2)*((1-mu)/d1**5+mu/d2**5)
    J = zeros((6,6))
    J[0,3] = 1
    J[1,4] = 1
    J[2,5] = 1
